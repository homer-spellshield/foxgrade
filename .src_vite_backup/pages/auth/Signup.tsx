...
export default function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setSuccess(true);
      // Wait a moment then navigate to onboarding (if auto-login works) or login page
      setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF8] flex items-center justify-center p-6 text-slate-900">
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-8 w-full max-w-md">
        ...
        <p className="text-center text-slate-500 mb-6 text-sm">Set up your organization's Trust Centre</p>
        ...
        {success ? (
          <div className="bg-emerald-50 text-emerald-700 p-4 rounded-md text-center border border-emerald-200">
            <h3 className="font-medium mb-1">Account created!</h3>
            <p className="text-sm">Redirecting you to setup...</p>
          </div>
        ) : (
        ...
        <div className="mt-6 text-center text-sm text-slate-600">
          ...
          <Link to="/login" className="text-[#1A3D2E] font-medium hover:underline">
            Log in
          </Link>
        </div>
      </div>
